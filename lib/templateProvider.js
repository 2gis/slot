...
function getStorage(ns) {
    ...
    if (env.isServer) {
        try {
            return env.requirePrivate('templates')[ns];
        } catch (ex) {
...
function compileTemplates(templateSpecs, handlebars) {
    ...
    var id = templateSpecs.id || templateSpecNextId++;
    ...
    if (!(id in handlebars._compiledTemplates)) {
        ...
        _.each(templateSpecs, function(spec, name) {
            if (typeof spec == 'function') {
                // old format
                templates[name] = handlebars.template({compiler: [7,'>= 4.0.0'], main: spec});
            } else {
...
exports.resolve = function(ns, path, handlebars) {
    ...
    if (!(cacheKey in handlebars._compiledTemplates)) {
        ...
        if (typeof spec == 'function') {
            // old format
            handlebars._compiledTemplates[cacheKey] = handlebars.template({compiler: [7,'>= 4.0.0'], main: spec});
        } else {
